...
def match_answer_span(p, answer, tokenizer):
    tokens = tokenizer.tokenize(p)
    text = tokens.words(uncased=False)
    matched = set()

    for single_answer in answer:
        single_answer = normalize(single_answer)
        single_answer = tokenizer.tokenize(single_answer)
        single_answer = single_answer.words(uncased=False)
...
def read_mrqa_examples(input_file, is_training):
    ...
    for i, entry in tqdm(enumerate(input_data), total=len(input_data)):
        ...
        paragraph_text = "yes no " + paragraph_text.strip()
        ...
        for qa in entry["qas"]:
            ...
            if is_training:
                ...
                try:
                    ...
                    matched_answers = [answers[0]]
                
                char_starts = []
                char_ends = []
                for  matched in matched_answers:
                    char_starts += [i for i in range(len(paragraph_text)) if paragraph_text.startswith(matched, i)]
                    char_ends += [start + len(matched) - 1 for start in char_starts]
                spans = list(zip(char_starts, char_ends))
                spans = sorted(spans)
                assert(len(spans)) > 0
...
def get_raw_scores(dataset, predictions):
    ...
    for example in dataset:
        for qa in example['qas']:
            answers[qa['qid']] = [qa['detected_answers'][0]['answer_text']]
    exact_scores = {}
    f1_scores = {}
    ...
    if args.do_train:
        ...
        for lr in lrs:
            ...
            if not args.finetuning_dir is None:
                # load model
                model = BertForQuestionAnswering.from_pretrained(args.finetuning_dir)
                if args.fp16:
                    model.half()
                model.to(device)
            else:
...
if __name__ == "__main__":
        ...
        parser.add_argument("--finetuning_dir", default=None, type=str, help="load model to resume training")
